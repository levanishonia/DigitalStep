...
export function Field(props: TextInputProps) { useTheme(); const styles = makeStyles(); return <TextInput placeholderTextColor={colors.placeholder} style={[styles.input, props.multiline && styles.multiline]} {...props} />; }
export function Badge({ label, tone = 'neutral' }: { label: string; tone?: 'neutral' | 'success' | 'warning' | 'danger' | 'info' | 'purple' }) { useTheme(); const styles = makeStyles(); const palette = tone === 'success' ? [colors.success, colors.successSoft] : tone === 'warning' ? [colors.accent, colors.accentSoft] : tone === 'danger' ? [colors.danger, colors.dangerSoft] : tone === 'info' ? [colors.info, colors.infoSoft] : tone === 'purple' ? [colors.purple, colors.purpleSoft] : [colors.muted, colors.surface]; return <View style={[styles.badge, { backgroundColor: palette[1] }]}><Text style={[styles.badgeText, { color: palette[0] }]}>{label}</Text></View>; }
export function Row({ title, detail, icon = 'ellipse', right }: { title: string; detail?: string; icon?: IconName; right?: ReactNode }) { useTheme(); const styles = makeStyles(); return <Card><View style={styles.row}><IconCircle name={icon} /><View style={{ flex: 1 }}><Text style={styles.rowTitle}>{title}</Text>{detail ? <Text style={styles.rowDetail}>{detail}</Text> : null}</View>{right}</View></Card>; }
export function Skeleton() { useTheme(); const styles = makeStyles(); return <Card><View style={styles.skeletonTitle} /><View style={styles.skeletonLine} /><View style={[styles.skeletonLine, { width: '55%' }]} /></Card>; }
...
const makeStyles = () => StyleSheet.create({
  ...
  card: { backgroundColor: colors.card, borderRadius: 20, borderWidth: 1, borderColor: colors.border, padding: spacing.md, shadowColor: colors.shadow, shadowOpacity: 0.06, shadowRadius: 14, shadowOffset: { width: 0, height: 8 } },
  ...
  secondaryButton: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, shadowOpacity: 0.03 },
