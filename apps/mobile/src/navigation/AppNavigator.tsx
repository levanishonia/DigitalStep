...
import { AnalyticsScreen, BusinessOnboardingScreen, CalendarScreen, CampaignsScreen, HomeScreen, LoginScreen, PlannerScreen, RecommendationsScreen, TemplatesScreen, RegisterScreen, SettingsScreen, TasksScreen, WelcomeScreen, WeeklyPlanScreen, MoreScreen } from '../screens/screens';

const Stack = createNativeStackNavigator();
const Tabs = createBottomTabNavigator();

const tabIcons: Record<string, keyof typeof Ionicons.glyphMap> = {
  HomeTab: 'home', TasksTab: 'checkmark-circle', PlannerTab: 'calendar', AnalyticsTab: 'bar-chart', MoreTab: 'menu'
};
...
function PlannerStack() { const { t } = useI18n(); return <Stack.Navigator><Stack.Screen name="PlannerMain" component={WeeklyPlanScreen} options={{ headerShown: false }} /><Stack.Screen name="Calendar" component={CalendarScreen} options={nestedOptions(t('nav.calendar'))} /><Stack.Screen name="ContentPlanner" component={PlannerScreen} options={nestedOptions(t('nav.contentPlanner'))} /></Stack.Navigator>; }
...
function MainTabs() {
  ...
  return (
    <Tabs.Navigator screenOptions={({ route }) => ({
      ...
      <Tabs.Screen name="PlannerTab" component={PlannerStack} />
