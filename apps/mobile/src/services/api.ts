...
export function getTasks(token: string, filters: { status?: TaskStatus | 'all'; startDate?: string; endDate?: string } = {}) {
  ...
  return api<{ tasks: Task[] }>(`/tasks${query ? `?${query}` : ''}`, {}, token);
}
...
export function getContentItems(token: string, filters: { status?: ContentStatus | 'all'; type?: ContentType | 'all'; startDate?: string; endDate?: string } = {}) {
  const params = new URLSearchParams();
  if (filters.status && filters.status !== 'all') params.set('status', filters.status);
