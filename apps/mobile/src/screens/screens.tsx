...
export function DSStudioScreen() {
  ...
  async function generate() { if (!token) return; setGenerating(true); setNotice(''); try { const content = await generateStudioContent({ businessId: form.businessId, contentType: form.contentType, platform: form.platform, goal: form.goal, tone: form.tone, language: form.language }, token); setGenerated(content); } catch (err) { setNotice(cleanApiError(err)); } finally { setGenerating(false); } }
