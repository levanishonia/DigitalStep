...
import { BusinessInput, Language, ContentItem, ContentItemInput, ContentStatus, ContentType, DashboardResponse, MarketingChannel, Task, TaskInput, TaskPriority, TaskStatus, SocialAccount, SocialAccountInput, SocialPlatform, StudioGeneratedContent, WeeklyPlanResponse, acceptWeeklyPlan, createContentItem, createSocialAccount, createTask, deleteContentItem, deleteSocialAccount, deleteTask, getCalendar, getCampaigns, getContentItems, getDashboard, getSocialAccounts, getTasks, generateStudioContent, getBusinesses, getWeeklyPlan, updateBusinessContentLanguage, updateContentItem, updateSocialAccount, updateTask } from '../services/api';
...
const socialPlatforms: { label: string; value: SocialPlatform; icon: IconName }[] = [{ label: 'Facebook', value: 'facebook', icon: 'logo-facebook' }, { label: 'Instagram', value: 'instagram', icon: 'logo-instagram' }, { label: 'TikTok', value: 'tiktok', icon: 'logo-tiktok' }, { label: 'LinkedIn', value: 'linkedin', icon: 'logo-linkedin' }, { label: 'Website', value: 'website', icon: 'globe' }, { label: 'Email Marketing', value: 'email_marketing', icon: 'mail' }];
const socialPlatformMeta = Object.fromEntries(socialPlatforms.map((platform) => [platform.value, platform])) as Record<SocialPlatform, { label: string; value: SocialPlatform; icon: IconName }>;
function labelSocialPlatform(platform: SocialPlatform) { return socialPlatformMeta[platform]?.label ?? humanizeLabel(platform); }
...
export function RegisterScreen() {
  ...
  async function submit() {
    setError('');
    if (!name.trim() || !email.trim() || !password) return setError(t('error.registerFields'));
    if (password.length < 8) return setError(t('error.passwordLength'));
    setLoading(true);
...
export function BusinessOnboardingScreen({ navigation }: any) {
  ...
  async function submit() {
    setError('');
    if (!canSubmit) return setError(t('error.businessDetails'));
    setLoading(true);
    try {
...
export function HomeScreen({ navigation }: any) {
  ...
  if (loading) return <Screen title={t('dashboard.title')} subtitle={t('dashboard.loading')}><Skeleton /><Skeleton /><Skeleton /></Screen>;
  const business = dashboard?.business;
  return <Screen title={`${greeting}, ${user?.name?.split(' ')[0] ?? business?.name ?? t('greeting.there')} 👋`} subtitle={business ? `${business.name} · ${t('dashboard.subtitleBusiness')}` : t('dashboard.subtitleDefault')} refreshing={loading} onRefresh={reload}><ErrorMessage message={error} />{error ? <Button secondary label={t('common.tryAgain')} icon="refresh" onPress={reload} /> : null}<View style={{ flexDirection: 'row', gap: 8 }}><View style={{ flex: 1 }}><SummaryCard label={t('dashboard.dueToday')} value={dashboard?.tasks.length ?? 0} icon="today" tone="primary" /></View><View style={{ flex: 1 }}><SummaryCard label={t('dashboard.upcoming')} value={dashboard?.contentItems.length ?? 0} icon="paper-plane" tone="info" /></View><View style={{ flex: 1 }}><SummaryCard label={t('settings.channels')} value={dashboard?.socialAccounts.length ?? 0} icon="share-social" tone="accent" /></View></View><Section title={t('dashboard.quickActions')}><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button label={t('dashboard.createTask')} icon="add-circle" onPress={() => navigation.navigate('TasksTab', { screen: 'TasksMain' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label={t('dashboard.createContent')} icon="create" onPress={() => navigation.navigate('MoreTab', { screen: 'ContentPlanner' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label={t('dashboard.browseTemplates')} icon="library" onPress={() => navigation.navigate('MoreTab', { screen: 'Templates' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label={t('dashboard.createCampaign')} icon="megaphone" onPress={() => navigation.navigate('AnalyticsTab', { screen: 'Campaigns' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label={t('dashboard.openCalendar')} icon="calendar" onPress={() => navigation.navigate('MoreTab', { screen: 'Calendar' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label={t('nav.socialAccounts')} icon="share-social" onPress={() => navigation.navigate('MoreTab', { screen: 'SocialAccounts' })} /></View></View></Section><Section title={t('dashboard.todayTasks')}>{dashboard?.tasks.length ? dashboard.tasks.map((task) => <Row key={task.id} icon="checkmark-circle" title={task.title} detail={`${labelText(task.status)} · ${formatDate(task.dueDate)}${task.description ? ` · ${task.description}` : ''}`} right={<Badge label={labelText(task.priority)} tone={priorityTone[task.priority]} />} />) : <EmptyState icon="checkmark-done" title={t('dashboard.noTasks')} detail={t('dashboard.noTasksDetail')} />}</Section><Section title={t('dashboard.upcomingContent')}>{dashboard?.contentItems.length ? dashboard.contentItems.map((item) => <Row key={item.id} icon={channelIcon[item.channel]} title={item.title} detail={`${labelText(item.type)} · ${labelChannel(item.channel)} · ${formatDate(item.publishDate ?? item.scheduledFor)}`} right={<Badge label={labelText(item.status)} tone={statusTone[item.status]} />} />) : <EmptyState icon="images" title={t('dashboard.noUpcoming')} detail={t('dashboard.noUpcomingDetail')} />}</Section><Section title={t('dashboard.activeChannels')}>{dashboard?.socialAccounts.length ? dashboard.socialAccounts.map((account) => <Row key={account.id} icon={socialPlatformMeta[account.platform].icon} title={labelSocialPlatform(account.platform)} detail={account.url || account.username || t('dashboard.connectedChannel')} right={<Badge label={account.isPrimary ? t('label.primary') : t('label.active')} tone={account.isPrimary ? 'purple' : 'success'} />} />) : <EmptyState icon="share-social" title={t('dashboard.noChannels')} detail={t('dashboard.noChannelsDetail')} />}</Section><Section title={t('dashboard.activeCampaigns')}>{dashboard?.campaigns.length ? dashboard.campaigns.map((campaign) => <Row key={campaign.id} icon="megaphone" title={campaign.name} detail={campaign.objective} right={<Badge label={labelText(campaign.status)} tone={statusTone[campaign.status]} />} />) : <EmptyState icon="flag" title={t('dashboard.noCampaigns')} detail={t('dashboard.noCampaignsDetail')} />}</Section><Section title={t('dashboard.recommendations')}>{dashboard?.recommendations.length ? dashboard.recommendations.map((recommendation) => <Row key={recommendation.id} icon="bulb" title={recommendation.title} detail={recommendation.description} right={<Badge label={`P${recommendation.priority}`} tone="warning" />} />) : <EmptyState icon="bulb" title={t('dashboard.noRecommendations')} detail={t('dashboard.noRecommendationsDetail')} />}</Section></Screen>;
}
...
type GeneratedContent = StudioGeneratedContent;

type StudioForm = { businessId: string; businessType: string; contentType: StudioContentType; platform: StudioPlatform; goal: StudioGoal; tone: StudioTone; language: Language };
...
export function DSStudioScreen() {
  ...
  const [form, setForm] = useState<StudioForm>({ businessId: '', businessType: '', contentType: 'post', platform: 'instagram', goal: 'sales', tone: 'friendly', language });
  const [generated, setGenerated] = useState<GeneratedContent | null>(null);
  const [saving, setSaving] = useState(false);
  const [generating, setGenerating] = useState(false);
  const [notice, setNotice] = useState('');
  useEffect(() => { setForm((current) => ({ ...current, language })); }, [language]);
  useEffect(() => { if (!token) return; getBusinesses(token).then(({ businesses }) => { const business = businesses[0]; if (business) setForm((current) => ({ ...current, businessId: business.id, businessType: current.businessType || business.industry || business.name })); }).catch(() => undefined); }, [token]);
  async function generate() { if (!token) return; setGenerating(true); setNotice(''); try { const content = await generateStudioContent({ businessId: form.businessId, businessType: form.businessType.trim(), contentType: form.contentType, platform: form.platform, goal: form.goal, tone: form.tone, language: form.language }, token); setGenerated(content); } catch (err) { setNotice(cleanApiError(err)); } finally { setGenerating(false); } }
...
export function TemplatesScreen({ navigation }: any) {
  ...
  async function useTemplate(template: ContentTemplate) {
    ...
    try {
      ...
      navigation.navigate('MoreTab', { screen: 'ContentPlanner' });
...
export function SocialAccountsScreen() {
  ...
  const [form, setForm] = useState<SocialAccountInput>({ platform: 'facebook', username: '', url: '', isPrimary: false, isActive: true });
  const [editingId, setEditingId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const load = useCallback(async () => { if (!token) return; setLoading(true); setError(''); try { setAccounts((await getSocialAccounts(token)).socialAccounts); } catch (err) { setError(cleanApiError(err)); } finally { setLoading(false); } }, [token]);
  useFocusEffect(useCallback(() => { void load(); }, [load]));
  function edit(account: SocialAccount) { setEditingId(account.id); setForm({ platform: account.platform, username: account.username ?? '', url: account.url ?? '', isPrimary: account.isPrimary, isActive: account.isActive }); }
  function reset() { setEditingId(null); setForm({ platform: 'facebook', username: '', url: '', isPrimary: accounts.length === 0, isActive: true }); }
  async function save() { if (!token) return; setSaving(true); setError(''); try { const input = { ...form, username: form.username?.trim(), url: form.url?.trim() }; if (editingId) await updateSocialAccount(editingId, input, token); else await createSocialAccount(input, token); reset(); await load(); } catch (err) { setError(cleanApiError(err)); } finally { setSaving(false); } }
  async function remove(id: string) { if (!token) return; setError(''); try { await deleteSocialAccount(id, token); await load(); if (editingId === id) reset(); } catch (err) { setError(cleanApiError(err)); } }
  const needsUrl = form.platform === 'website';
  const needsProvider = form.platform === 'email_marketing';
  return <Screen title="Social Accounts" subtitle="Store marketing channels now, without connecting real social APIs yet." refreshing={loading} onRefresh={load}><ErrorMessage message={error} /><Section title={editingId ? 'Edit channel' : 'Add channel'}><Card><Text style={{ color: colors.text, fontWeight: '800' }}>Platform</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>{socialPlatforms.map((platform) => <Chip key={platform.value} label={platform.label} value={platform.value} selected={form.platform === platform.value} onPress={(value) => setForm((current) => ({ ...current, platform: value }))} />)}</View><Field placeholder={needsProvider ? 'Provider name (Mailchimp, Klaviyo...)' : needsUrl ? 'Website name (optional)' : 'Username or page name'} value={form.username ?? ''} onChangeText={(username) => setForm((current) => ({ ...current, username }))} /><Field placeholder={needsUrl ? 'Website URL' : 'URL (optional)'} autoCapitalize="none" autoCorrect={false} keyboardType="url" value={form.url ?? ''} onChangeText={(url) => setForm((current) => ({ ...current, url }))} /><Row icon="radio-button-on" title="Active" detail="Show this channel in dashboard summaries." right={<Switch value={form.isActive} onValueChange={(isActive) => setForm((current) => ({ ...current, isActive }))} />} /><Row icon="star" title="Primary platform" detail="Mark this as the main marketing channel." right={<Switch value={form.isPrimary} onValueChange={(isPrimary) => setForm((current) => ({ ...current, isPrimary }))} />} /><Button label={editingId ? 'Save changes' : 'Add channel'} icon="save" loading={saving} onPress={save} />{editingId ? <Button secondary label="Cancel editing" icon="close" onPress={reset} /> : null}</Card></Section><Section title="Connected channels">{accounts.length ? accounts.map((account) => <Card key={account.id} elevated><View style={{ flexDirection: 'row', gap: 12 }}><IconCircle name={socialPlatformMeta[account.platform].icon} /><View style={{ flex: 1 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 8 }}><Text style={{ color: colors.text, fontWeight: '900', fontSize: 17, flex: 1 }}>{labelSocialPlatform(account.platform)}</Text><Badge label={account.isActive ? 'Active' : 'Inactive'} tone={account.isActive ? 'success' : 'neutral'} /></View><Text style={{ color: colors.muted, marginTop: 6 }}>{account.url || account.username || 'No account details added'}</Text>{account.isPrimary ? <Badge label="Primary" tone="purple" /> : null}<View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}><View style={{ flex: 1 }}><Button secondary label="Edit" icon="create" onPress={() => edit(account)} /></View><View style={{ flex: 1 }}><Button secondary label="Delete" icon="trash" onPress={() => remove(account.id)} /></View></View></View></View></Card>) : loading ? <ActivityIndicator color={colors.primary} /> : <EmptyState icon="share-social" title="No channels yet" detail="Add Facebook, Instagram, TikTok, LinkedIn, website, or email marketing details." />}</Section></Screen>;
}
...
export function MoreScreen({ navigation }: any) {
  ...
  const items = [
    ...
    { route: 'SocialAccounts', icon: 'share-social' as IconName, title: t('nav.socialAccounts'), detail: t('more.socialAccountsDetail') },
...
export function SettingsScreen() {
  ...
  useEffect(() => {
    let mounted = true;
    loadReminderState().then((state) => { if (mounted) setReminder(state); }).catch(() => { if (mounted) setReminderError(t('reminders.loadError')); });
  ...
  async function toggleReminder(enabled: boolean) {
    ...
    try {
      ...
      setReminder(next);
      if (enabled && next.notificationStatus !== 'granted') setReminderError(t('reminders.deviceDisabled'));
    } catch {
      setReminderError(t('reminders.updateError'));
    } finally {
  ...
  async function changeReminderTime(time: string) {
    ...
    try {
      ...
      setReminderError(t('reminders.timeError'));
    } finally {
      setReminderSaving(false);
    }
  }
...
function DailyReminderCard({ reminder, reminderTimes, saving, error, onToggle, onChangeTime }: { reminder: ReminderState; reminderTimes: string[]; saving: boolean; error: string; onToggle: (enabled: boolean) => void; onChangeTime: (time: string) => void }) {
  ...
  return <Card><View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><IconCircle name="notifications" /><View style={{ flex: 1 }}><Text style={{ color: colors.text, fontWeight: '900', fontSize: 16 }}>{translate('reminders.enable')}</Text><Text style={{ color: colors.muted, marginTop: 4, lineHeight: 20 }}>{reminder.enabled ? `${translate('reminders.detailOn')} ${formatReminderTime(reminder.time)}.` : translate('reminders.detailOff')}</Text></View><Switch value={reminder.enabled} disabled={saving || reminder.notificationStatus === 'unavailable'} onValueChange={onToggle} trackColor={{ false: colors.border, true: colors.primarySoft }} thumbColor={reminder.enabled ? colors.primary : colors.muted} /></View><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 14, alignItems: 'center' }}><Badge label={status} tone={statusTone} /><Badge label={reminder.enabled ? translate('reminders.on') : translate('reminders.off')} tone={reminder.enabled ? 'success' : 'neutral'} /></View><Text style={{ color: colors.text, fontWeight: '800', marginTop: 16 }}>{translate('reminders.time')}</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>{reminderTimes.map((time) => <Chip key={time} label={formatReminderTime(time)} value={time} selected={reminder.time === time} onPress={onChangeTime} />)}</View><Text style={{ color: colors.muted, marginTop: 12, lineHeight: 20 }}>{translate('reminders.localOnly')}</Text><ErrorMessage message={error} /></Card>;
}
