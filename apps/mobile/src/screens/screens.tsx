...
export function HomeScreen({ navigation }: any) {
  ...
  return <Screen title={`${greeting}, ${user?.name?.split(' ')[0] ?? business?.name ?? 'there'} 👋`} subtitle={business ? `${business.name} marketing command center.` : 'Your marketing at a glance.'} refreshing={loading} onRefresh={reload}><ErrorMessage message={error} />{error ? <Button secondary label="Try again" icon="refresh" onPress={reload} /> : null}<View style={{ flexDirection: 'row', gap: 8 }}><View style={{ flex: 1 }}><SummaryCard label="Due today" value={dashboard?.tasks.length ?? 0} icon="today" tone="primary" /></View><View style={{ flex: 1 }}><SummaryCard label="Upcoming" value={dashboard?.contentItems.length ?? 0} icon="paper-plane" tone="info" /></View><View style={{ flex: 1 }}><SummaryCard label="Channels" value={dashboard?.socialAccounts.length ?? 0} icon="share-social" tone="accent" /></View></View><Section title="Quick actions"><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button label="Create Task" icon="add-circle" onPress={() => navigation.navigate('TasksTab', { screen: 'TasksMain' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label="Create Content" icon="create" onPress={() => navigation.navigate('MoreTab', { screen: 'ContentPlanner' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label="Browse Templates" icon="library" onPress={() => navigation.navigate('MoreTab', { screen: 'Templates' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label="Create Campaign" icon="megaphone" onPress={() => navigation.navigate('AnalyticsTab', { screen: 'Campaigns' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label="Open Calendar" icon="calendar" onPress={() => navigation.navigate('MoreTab', { screen: 'Calendar' })} /></View><View style={{ flexBasis: '48%', flexGrow: 1 }}><Button secondary label="Social Accounts" icon="share-social" onPress={() => navigation.navigate('MoreTab', { screen: 'SocialAccounts' })} /></View></View></Section><Section title="Today’s marketing tasks">{dashboard?.tasks.length ? dashboard.tasks.map((task) => <Row key={task.id} icon="checkmark-circle" title={task.title} detail={`${labelText(task.status)} · ${formatDate(task.dueDate)}${task.description ? ` · ${task.description}` : ''}`} right={<Badge label={labelText(task.priority)} tone={priorityTone[task.priority]} />} />) : <EmptyState icon="checkmark-done" title="No tasks due today" detail="Enjoy the breathing room or create a quick action for tomorrow." />}</Section><Section title="Upcoming content">{dashboard?.contentItems.length ? dashboard.contentItems.map((item) => <Row key={item.id} icon={channelIcon[item.channel]} title={item.title} detail={`${labelText(item.type)} · ${labelChannel(item.channel)} · ${formatDate(item.publishDate ?? item.scheduledFor)}`} right={<Badge label={labelText(item.status)} tone={statusTone[item.status]} />} />) : <EmptyState icon="images" title="No upcoming content" detail="Planned posts, emails, and website updates will show up here." />}</Section><Section title="Active marketing channels">{dashboard?.socialAccounts.length ? dashboard.socialAccounts.map((account) => <Row key={account.id} icon={socialPlatformMeta[account.platform].icon} title={labelSocialPlatform(account.platform)} detail={account.url || account.username || 'Connected channel'} right={<Badge label={account.isPrimary ? 'Primary' : 'Active'} tone={account.isPrimary ? 'purple' : 'success'} />} />) : <EmptyState icon="share-social" title="No active channels" detail="Add social accounts from More to summarize connected marketing channels." />}</Section><Section title="Active campaigns">{dashboard?.campaigns.length ? dashboard.campaigns.map((campaign) => <Row key={campaign.id} icon="megaphone" title={campaign.name} detail={campaign.objective} right={<Badge label={labelText(campaign.status)} tone={statusTone[campaign.status]} />} />) : <EmptyState icon="flag" title="No active campaigns" detail="Create campaigns when you are ready to track focused marketing pushes." />}</Section><Section title="Recommendations">{dashboard?.recommendations.length ? dashboard.recommendations.map((recommendation) => <Row key={recommendation.id} icon="bulb" title={recommendation.title} detail={recommendation.description} right={<Badge label={`P${recommendation.priority}`} tone="warning" />} />) : <EmptyState icon="bulb" title="No recommendations yet" detail="DigitalStep will surface practical suggestions as your plan fills in." />}</Section></Screen>;
}
...
export function MoreScreen({ navigation }: any) {
  ...
  const items = [
    ...
    { route: 'SocialAccounts', icon: 'share-social' as IconName, title: 'Social Accounts', detail: 'Manage connected marketing channels and primary platform.' },
